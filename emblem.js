...
(function() {
    ...
    // Get static pre-rendered emblem PNG URL from parameters
    // Uses Cloudflare Worker for on-demand emblem generation
    window.getStaticEmblemUrl = function(params) {
        ...
        return `https://h2emblem.carnagereport.workers.dev/P${P}-S${S}-EP${EP}-ES${ES}-EF${EF}-EB${EB}-ET${ET}.png`;
    }

    // Parse emblem params from halo2pc.com style URL
    window.parseEmblemParams = function(url) {
        if (!url) return null;
        ...
        try {
            ...
            // Must have at least EF parameter to be valid
            if (!urlParams.has('EF') && !urlParams.has('P')) return null;

            return {
                P: parseInt(urlParams.get('P') || 10),
                S: parseInt(urlParams.get('S') || 0),
                EP: parseInt(urlParams.get('EP') || 0),
                ES: parseInt(urlParams.get('ES') || 1),
    ...
    // Get static emblem URL from halo2pc.com style URL
    window.getStaticEmblemUrlFromEmblemUrl = function(emblemUrl) {
        const params = window.parseEmblemParams(emblemUrl);
